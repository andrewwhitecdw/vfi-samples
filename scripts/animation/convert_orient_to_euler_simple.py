...
def detect_gimbal_lock(euler_angles, order="XYZ", threshold=85.0):
    ...
    # Gimbal lock occurs when the middle rotation is near ±90°.
    # Normalize to [-180, 180] so wrapped angles such as 450° or -270°
    # (which are coterminal with 90°) are detected correctly.
    normalized = ((middle_rotation + 180.0) % 360.0) - 180.0
    return abs(abs(normalized) - 90.0) < (90.0 - threshold)
